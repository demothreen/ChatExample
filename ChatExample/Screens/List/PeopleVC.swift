...
class PeopleVC: UIViewController {
  ...
  private func setupCollectionView() {
    ...
    collectionView.registerCell(UserCell.self)
  }
  ...
  private func createDataSource() {
    dataSource = UICollectionViewDiffableDataSource<Section, MUser>(collectionView: collectionView, cellProvider: { (collectionView, indexPath, user) -> UICollectionViewCell? in
      ...
      switch section {
      case .users:
        return collectionView.configure(cellType: UserCell.self, with: user, for: indexPath)
      }
    })
    ...
    dataSource?.supplementaryViewProvider = {
      ...
      sectionHeader.configure(text: section.description(usersCount: items.count), fontSize: 36, textColor: .black)
